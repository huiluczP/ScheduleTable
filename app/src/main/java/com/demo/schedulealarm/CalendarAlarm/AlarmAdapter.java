package com.demo.schedulealarm.CalendarAlarm;
...
public class AlarmAdapter {
    ...
    private static String CALENDARS_NAME = "jacky";
    private static String CALENDARS_ACCOUNT_NAME = "jacky_account";
    private static String CALENDARS_ACCOUNT_TYPE = "com.demo.schedulealarm";
    private static String CALENDARS_DISPLAY_NAME = "JACKY账户";
    ...
    static {
     ...
     //检查是否已经添加了日历账户，如果没有添加先添加一个日历账户再查询，返回id值
    ...
    //日历事件添加方法
    public static void addCalendarEvent(Context context, String title, String description, long reminderTime, int previousDate,ScheduleInfo info) {
        if (context == null) {
            return;
        }
        ...
        mCalendar.set(Calendar.MINUTE,info.getMinute());
        mCalendar.set(info.getYear(),info.getMounth()-1,info.getDate(),info.getHour(),info.getMinute());

        //mCalendar.setTimeInMillis(reminderTime);//设置开始时间
        long start = mCalendar.getTimeInMillis();
        Log.i("++time+++",start+" ");
        mCalendar.setTimeInMillis(starts);
        start=mCalendar.getTime().getTime();
        mCalendar.setTimeInMillis(start + 30 * 60 * 1000);//设置终止时间，开始时间加30分钟
        long end = mCalendar.getTime().getTime();
        Log.i("++time+++",end+"");

        ContentValues event = new ContentValues();
        event.put("title", info.getTitle());
        event.put("description", info.getContent());
        ...
        values.put(CalendarContract.Reminders.MINUTES, 0);// 提前5分钟有提醒
